...
public:
	...
	void OpenAssetEditor( const TArray<UObject*>& InObjects, const EAssetTypeActivationOpenedMethod OpenedMethod, TSharedPtr<IToolkitHost> EditWithinLevelEditor ) override {}
	bool AssetsActivatedOverride( const TArray<UObject*>& InObjects, EAssetTypeActivationMethod::Type ActivationType ) override {
		return false;
	}
	bool CanRename( const FAssetData& InAsset, FText* OutErrorMsg ) const override {
		return true;
	}
	bool CanDuplicate( const FAssetData& InAsset, FText* OutErrorMsg ) const override {
		return false;
	}
	TArray<FAssetData> GetValidAssetsForPreviewOrEdit( TArrayView<const FAssetData> InAssetDatas, bool bIsPreview ) override {
		return TArray<FAssetData>();
	}
	bool CanFilter() override {
		return false;
	}
	FName GetFilterName() const override {
		return FName();
	}
	...
	FString GetObjectDisplayName( UObject* Object ) const override {
		return FString("InkAsset");
	}
	...
	UThumbnailInfo* GetThumbnailInfo( UObject* Asset ) const override {
		return nullptr;
	}
	...
	void GetSourceFileLabels( const TArray<UObject*>& TypeAssets, TArray<FString>& OutSourceFileLabels ) const override {
