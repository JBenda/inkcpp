...
namespace ink::runtime::internal::casting {
	...
	public:
		...
		static constexpr size_t N = static_cast<size_t>(value_type::END);
		value_type _data[N][N];
	};

	template<value_type t1 = value_type::BEGIN, value_type t2 = value_type::BEGIN>
	constexpr void set_cast (value_type data[static_cast<size_t>(value_type::END)][static_cast<size_t>(value_type::END)]){
		if constexpr (t2 == value_type::END) {
		} else if constexpr (t1 == value_type::END) {
