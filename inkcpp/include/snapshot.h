...
namespace ink::runtime
{
	 ...
	 * For convinience there exist @ref globals_interface::create_snapshot() and runner_interface::create_snapshot() .
	 * If the runner is assoziated to the globals the snapshot will be identical.
	 * If multiple runners are assoziated to the same globals all will be contained,
	 * and cann be reconsrtucted with the id parameter of @ref story::new_runner_from_snapshot()
	 *
	 * @todo Currently the id is equal to the creation order, a way to name the single runner/threads is WIP
	 */
	class snapshot {
	public:
		virtual ~snapshot() {};
