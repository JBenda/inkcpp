...
{
public:
	list_impl(list_table& table, int lid)
	    : list_interface(table, lid)
	{
	}
	...
	list_interface::iterator begin() const override { return ++new_iterator(nullptr, 0); }

	list_interface::iterator begin(const char* list_name) const override;

	list_interface::iterator end() const override { return new_iterator(nullptr, -1); }
