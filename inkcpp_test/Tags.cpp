	...
	GIVEN("a story with tags")
	{
		...
		WHEN("second line")
		{
			...
			THEN("one tag")
			{
				...
				REQUIRE(thread->has_tags() == true);
				REQUIRE(thread->num_tags() == 1);
				REQUIRE(std::string(thread->get_tag(0)) == "tagged");
			}
		}
		WHEN("third line")
		{
			...
			THEN("two tags")
			{
				...
				REQUIRE(std::string(thread->get_tag(0)) == "tag next line");
				REQUIRE(std::string(thread->get_tag(1)) == "more tags");
			}
		}
		WHEN("fourth line")
		{
			...
			THEN("three tags")
			{
				...
				REQUIRE(std::string(thread->get_tag(0)) == "above");
				REQUIRE(std::string(thread->get_tag(1)) == "side");
				REQUIRE(std::string(thread->get_tag(2)) == "across");
			}
		}
		WHEN("print choices")
		{
