...
SCENARIO("simple_restorable_stack can be pushed and popped", "[stack][unit][internals]")
{
	...
	GIVEN("An empty stack")
	{
		...
		WHEN("items are added")
		{
			...
			THEN("the correct item is at the top") { REQUIRE(stack.top() == 3); }
			...
			WHEN("they are popped")
			{
				...
				THEN("the correct item is popped") { REQUIRE(pop == 3); }

				THEN("there are fewer items") { REQUIRE(stack.size() == 2); }

				THEN("there is a new item on top") { REQUIRE(stack.top() == 2); }
...
SCENARIO("simple_restorable_stack supports save/restore", "[stack][unit][internals]")
{
	GIVEN("a stack with a few items that has been saved")
	{
		...
		auto check_restore = [&stack]() {
			WHEN("we restore the stack")
			{
				...
				THEN("we should be back to having 4 items") { REQUIRE(stack.size() == 3); }
		...
		WHEN("new items are pushed")
		{
			...
			THEN("the pushed item is on top") { REQUIRE(stack.top() == 4); }
			THEN("the size has increased") { REQUIRE(stack.size() == 4); }
